...
## Plot speed lines ####
interpolateLatLon <- function(d, id) {
	# generate spatial line from shape
	shape_line <- spTransform(SpatialLines(list(Lines(Line(rv$shapes[shape_id == id, c('shape_pt_lon', 'shape_pt_lat')]), ID = id)), CRS('+proj=longlat +ellps=WGS84')), CRS(st_crs(rv$crs)$proj4string))
...
speedOnLine <- function(avl, by_dist = 1) {
  ...
  expanded[, c('lon_imp', 'lat_imp') := interpolateLatLon(avl_dist_traveled, shape_id), by = c('start_date', 'trip_id', 'shape_id')]
  return(expanded)
  }
